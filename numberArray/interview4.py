...
def input_array():
    ...
    if key == 'y' or key == 'Y':
        ...
        i = 0
        while i < m:
            line = input('please input the ' + str(i) +
                         ' row array, the col number is ' + str(n) + '(number splited with \',\'):')
            if line == '' and i < m:
                print('The array has ' + str(m) + ' row,'
                      + ' please input the rest ' + str(m - i) + ' row!')
                continue
            if len(line) != (n + n - 1):
                print('The input number is less ' + str(n)
                      + ' in a row, please input again!')
                continue
...
def main():
    ...
    if ret:
        print('The number %d is in the two-dimension array, and the local is in %d row, %d col.' %
              (val, local[0], local[1]))
    else:
