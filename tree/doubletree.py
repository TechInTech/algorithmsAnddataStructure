...
class Tree(object):
    ...
    def constructtree(self, preorder, inorder, k1, Lk1, k2, Lk2, p_node=None):
        rootval = preorder[k1]
        root = BinaryTreeNode(rootval, parent=p_node)
        ...
        if leftlength > 0:
            root.left_ = self.constructtree(
                preorder, inorder, k1 + 1, leftpreo, k2, rootino - 1, p_node=root)

        if leftlength < (Lk1 - k1):
            root.right_ = self.constructtree(
                preorder, inorder, leftpreo + 1, Lk1, rootino + 1, Lk2, p_node=root)
        return root
